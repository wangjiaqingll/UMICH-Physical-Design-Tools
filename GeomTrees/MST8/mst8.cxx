...
/* next, expand the box till we find something not at the search origin */
    ...
    for (i=0; i<nl; i++) {
        long int node = (long int)list[i]->ud;   // a potential 64-bit issue
    ...
    for( ; DB_get_next_xyitem_32(&item); ) {
        long int node = (long int)item->ud; // a potential 64-bit issue
        int dnew = abs(x0-pts[node].x)+abs(y0-pts[node].y);
        if (pts[node].where == 0) {
            pts[node].w = dnew;
            pts[node].other = id;
...
double mst()
{
    ...
    for(i=0; i<NPts; i++) {
        ...
        register_area_32(t, xp, yp,  xp, yp, (void *)(long int)i);
        pts[i].where = 0;
        }
...
void mst_package_done()
{
    ...
    free((char *)soln_arcs); soln_arcs = NULL;
...
#define P(a) ((long int)(((POINT *)a)->handle))

// a potential 64-bit issue
extern "C"
{
...
int handle_to_index(void *h)
{
    ...
    long int ih = (long int)h;    // a potential 64-bit issue
    if (NPts == 0 || ih < (long int)pts[0].handle || ih > (long int)pts[NPts-1].handle)
        return -1;
    for(; low <= high; ) {
        int test = (low + high)>>1;
        int it = (long int)pts[test].handle;
        if (it == ih)
            return test;
        else if (it < ih)
            low = test+1;
...
double mst_cost(const int& numPoints,const double* xArray,const double* yArray)
{
    int i,j;
//  char what;
    ...
    for(i=0; i<numPoints; i++) 
     add_new_point((int)xArray[i],(int)yArray[i],(void *)(long int)i);
    end_net();
    mst();
    j = 0;
