...
int versionCB(defwCallbackType_e c, defiUserData ud) {
  ...
  status = defwVersion(5, 2);
  CHECK_STATUS(status);
  return 0;

}
 
int casesensitiveCB(defwCallbackType_e c, defiUserData ud) {
  int status;

  checkType(c);
  if ((int)ud != userData) dataError();
  status = defwCaseSensitive("ON");
  CHECK_STATUS(status);
  return 0;
...
// DEFAULTCAP
int defaultCapCB(defwCallbackType_e c, defiUserData ud) {
  int status;
...
// IOTIMINGS
int iotimingCB(defwCallbackType_e c, defiUserData ud) {
  int status;
...
// SCANCHAIN
int scanchainCB(defwCallbackType_e c, defiUserData ud) {
  int status;
...
// CONSTRAINTS
int constraintCB(defwCallbackType_e c, defiUserData ud) {
  int status;
...
main(int argc, char** argv) {
  ...
  defwSetScanchainCbk (scanchainCB);
