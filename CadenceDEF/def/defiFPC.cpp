...
void defiFPC::clear() {
  ...
  for (i = 0; i < this->namesUsed_; i++) {
    if (this->names_[i]) free (this->names_[i]) ;
  }
  this->namesUsed_ = 0;
}
...
double defiFPC::min() const {
  return this->minMaxEqual_;
}


double defiFPC::max() const {
  return this->minMaxEqual_;
}
...
void defiFPC::addItem(char item, const char* name) {
  ...
  if (this->namesUsed_ >= this->namesAllocated_) {
    ...
    if (this->names_) free((char*)(this->names_));
    if (this->rowOrComp_) free(this->rowOrComp_);
    this->names_ = newN;
